...
if ($root == 1)
{
  $file =~ s/<language([^>]+)name="[^"]*"/<language$1name="Mako (HTML)"/s;
  $file =~ s/<language([^>]+)section="[^"]*"/<language$1section="Scripts"/s;
  $file =~ s/<language([^>]+)extensions="[^"]*"/<language$1extensions="*.mako"/s;
  $file =~ s/<language([^>]+)mimetype="[^"]*"/<language$1mimetype="text\/mako"/s;
}
else
{
  $file =~ s/<language([^>]+)name="([^"]*)"/<language$1name="$2\/Mako" hidden="true"/s;
...
$findmako = "<context name=\"FindMako\">\n<RegExpr context=\"##Mako/Mako\" String=\"(?:&lt;/?%|\$\{)\" lookAhead=\"true\" />\n<RegExpr context=\"##Mako/Mako\" String=\"^%\" lookAhead=\"true\" />\n</context>\n";

$file =~ s/<IncludeRules\s([^>]*)context="##(?!Alerts)([^"]+)"/<IncludeRules $1context="##$2\/Mako"/g;
$file =~ s/(<context\s[^>]*>)/$1\n<IncludeRules context="FindMako" \/>/g;
$file =~ s/(?=<\/contexts\s*>)/$findmako/;
